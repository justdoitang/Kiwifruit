...
namespace Kiwifruit.Infrastructure.Dapper
{
    public class DataAccess : IDataAccess
    {
        ...
        /// 日志记录
        /// </summary>
        private ILoggers _logger { get; set; }
        ...
        public bool CloseConnection(IDbConnection connection)
        {
            ...
            {
                result = false;
                _logger.Error(null, $"数据库连接关闭失败，错误信息：{ex.Message}");
                throw ex;
            }
            return result;
        }
        ...
        public T Execute<T>(string sql, object param, bool hasTransaction = false, CommandType commandType = CommandType.Text)
        {
            ...
            using (var connection = DbConnection())
            {
                ...
                if (hasTransaction)
                {
                    using (IDbTransaction transaction = connection.BeginTransaction(IsolationLevel.ReadCommitted))
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            _logger.Error(null, $"SQL语句：{sql}，使用系统事务执行 Execute 方法出错，错误信息：{ex.Message}");
                        }
                    }
                }
                else
                {
                    ...
                    {
                        _logger.Error(null, $"SQL语句：{sql}，不使用系统事务执行 Execute 方法出错，错误信息：{ex.Message}");
                    }
                }
            }
            return obj;
        ...
        public T Execute<T>(string sql, object param, IDbTransaction transaction, IDbConnection connection, CommandType commandType = CommandType.Text)
        {
            ...
            {
                _logger.Error(null, $"SQL语句：{sql}，使用外部事务执行 Execute 方法出错，错误信息：{ex.Message}");
                throw ex;
            }
            return obj;
        }
            ...
            using (var connection = DbConnection())
            {
                ...
                if (hasTransaction)
                {
                    using (IDbTransaction transaction = connection.BeginTransaction(IsolationLevel.ReadCommitted))
                    {
                        ...
                        {
                            transaction.Rollback();
                            _logger.Error(null, $"SQL语句：{sql}，使用系统事务执行 ExecuteAsync 方法出错，错误信息：{ex.Message}");
                        }
                    }
                }
                else
                {
                    ...
                    {
                        _logger.Error(null, $"SQL语句：{sql}，不使用系统事务执行 ExecuteAsync 方法出错，错误信息：{ex.Message}");
                    }
                }
            }
            return obj;
        ...
        public async Task<T> ExecuteAsync<T>(string sql, object param, IDbTransaction transaction, IDbConnection connection, CommandType commandType = CommandType.Text)
        {
            ...
            {
                _logger.Error(null, $"SQL语句：{sql}，使用外部事务执行 ExecuteAsync 方法出错，错误信息：{ex.Message}");
                throw ex;
            }
            return obj;
        }
            ...
            using (var connection = DbConnection())
            {
                ...
                if (hasTransaction)
                {
                    using (IDbTransaction transaction = connection.BeginTransaction(IsolationLevel.ReadCommitted))
                    {
                        ...
                        {
                            transaction.Rollback();
                            _logger.Error(null, $"SQL语句：{sql}，使用系统事务执行 ExecuteIList<T> 方法出错，错误信息：{ex.Message}");
                            throw ex;
                        }
                    }
                }
                else
                {
                    ...
                    {
                        _logger.Error(null, $"SQL语句：{sql}，不使用系统事务执行 ExecuteIList<T> 方法出错，错误信息：{ex.Message}");
                        throw ex;
                    }
                }
            }
        ...
        public IList<T> ExecuteIList<T>(string sql, object param, IDbTransaction transaction, IDbConnection connection, CommandType commandType = CommandType.Text)
        {
            ...
            {
                _logger.Error(null, $"SQL语句：{sql}，使用外部事务执行 ExecuteIList<T> 方法出错，错误信息：{ex.Message}");
                throw ex;
            }
            return list;
        }
            ...
            using (var connection = DbConnection())
            {
                ...
                if (hasTransaction)
                {
                    using (IDbTransaction transaction = connection.BeginTransaction(IsolationLevel.ReadCommitted))
                    {
                        ...
                        {
                            transaction.Rollback();
                            _logger.Error(null, $"SQL语句：{sql}，使用系统事务执行 ExecuteIListAsync<T> 方法出错，错误信息：{ex.Message}");
                            throw ex;
                        }
                    }
                }
                else
                {
                    ...
                    {
                        _logger.Error(null, $"SQL语句：{sql}，不使用系统事务执行 ExecuteIListAsync<T> 方法出错，错误信息：{ex.Message}");
                        throw ex;
                    }
                }
            }
        ...
        public async Task<IList<T>> ExecuteIListAsync<T>(string sql, object param, IDbTransaction transaction, IDbConnection connection, CommandType commandType = CommandType.Text)
        {
            ...
            {
                _logger.Error(null, $"SQL语句：{sql}，使用外部事务执行 ExecuteIListAsync<T> 方法出错，错误信息：{ex.Message}");
                throw ex;
            }
            return list;
        }
            ...
            using (var connection = DbConnection())
            {
                ...
                if (hasTransaction)
                {
                    using (IDbTransaction transaction = connection.BeginTransaction(IsolationLevel.ReadCommitted))
                    {
                        ...
                        {
                            transaction.Rollback();
                            _logger.Error(null, $"SQL语句：{sql}，使用系统事务执行 ExecuteNonQuery 方法出错，错误信息：{ex.Message}");
                            throw ex;
                        }
                    }
                }
                else
                {
                    ...
                    {
                        _logger.Error(null, $"SQL语句：{sql}，不使用系统事务执行 ExecuteNonQuery 方法出错，错误信息：{ex.Message}");
                        throw ex;
                    }
                }
            }
        ...
        public int ExecuteNonQuery(string sql, object param, IDbTransaction transaction, IDbConnection connection, CommandType commandType = CommandType.Text)
        {
            ...
            {
                _logger.Error(null, $"SQL语句：{sql}，使用外部事务执行 ExecuteNonQuery 方法出错，错误信息：{ex.Message}");
                throw ex;
            }
            return result;
        }
            ...
            using (var connection = DbConnection())
            {
                ...
                if (hasTransaction)
                {
                    using (IDbTransaction transaction = connection.BeginTransaction(IsolationLevel.ReadCommitted))
                    {
                        ...
                        {
                            transaction.Rollback();
                            _logger.Error(null, $"SQL语句：{sql}，使用系统事务执行 ExecuteNonQueryAsync 方法出错，错误信息：{ex.Message}");
                            throw ex;
                        }
                    }
                }
                else
                {
                    ...
                    {
                        _logger.Error(null, $"SQL语句：{sql}，不使用系统事务执行 ExecuteNonQueryAsync 方法出错，错误信息：{ex.Message}");
                        throw ex;
                    }
                }
            }
        ...
        public async Task<int> ExecuteNonQueryAsync(string sql, object param, IDbTransaction transaction, IDbConnection connection, CommandType commandType = CommandType.Text)
        {
            ...
            {
                _logger.Error(null, $"SQL语句：{sql}，使用外部事务执行 ExecuteNonQueryAsync 方法出错，错误信息：{ex.Message}");
                throw ex;
            }
            return result;
        }
        ...
        public T ExecuteScalar<T>(string sql, object param, bool hasTransaction = false, CommandType commandType = CommandType.Text)
        {
            ...
            using (var connection = DbConnection())
            {
                ...
                {
                    _logger.Error(null, $"SQL语句：{sql}，执行 ExecuteScalar<T> 方法出错，错误信息：{ex.Message}");
                    throw ex;
                }
            }
            return result;
        ...
        public async Task<T> ExecuteScalarAsync<T>(string sql, object param, bool hasTransaction = false, CommandType commandType = CommandType.Text)
        {
            ...
            using (var connection = DbConnection())
            {
                ...
                {
                    _logger.Error(null, $"SQL语句：{sql}，执行 ExecuteScalarAsync<T> 方法出错，错误信息：{ex.Message}");
                    throw ex;
                }
            }
            return result;
